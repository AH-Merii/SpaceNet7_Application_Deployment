...
def compose_arr(divide_img_ls, compose_img_dir, ext=".npy"):
    ...
    image = (
        np.zeros((cols * target_width, rows * target_height), dtype=np.float32) * 255
    )
    for y in range(rows):
        for x in range(cols):
            patch = np.load(im_list[cols * y + x])
            image[
                y * target_height : (y + 1) * target_height,
                x * target_width : (x + 1) * target_width,
            ] = patch
...
def create_label(root, f3x=True):
    ...
    for i, aoi in enumerate(aois):
        ...
        print(i, "aoi:", aoi)
        im_dir = os.path.join(
            root, aoi, "images_masked_3x/" if f3x else "images_masked/"
        )
...
def create_test_list(root):
    ...
    for aoi in os.listdir(root):
        ...
        for img_file in os.listdir(img_path):
            fw.write(
                os.path.join(aoi, "images_masked_3x_divide", img_file) + " dummy.tif\n"
            )
    fw.close()
